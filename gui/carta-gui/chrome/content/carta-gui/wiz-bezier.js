function wizInit() {
	...
	unit_y2 = "mm";
	edit_cpx1 = document.getElementById("edit-cpx1");
	unit_cpx1 = "mm";
	edit_cpy1 = document.getElementById("edit-cpy1");
	unit_cpy1 = "mm";
	edit_cpx2 = document.getElementById("edit-cpx2");
	unit_cpx2 = "mm";
	edit_cpy2 = document.getElementById("edit-cpy2");
	unit_cpy2 = "mm";
...
function wizValid() {
	if (!edit_x1.value.match(/^-?\d+$/) &&
		...
		return;
	} else if (!edit_cpx1.value.match(/^-?\d+$/) &&
		   !edit_cpx1.value.match(/^-?\d+\.\d+$/)) {
		alert("Mauvaise valeur de point de contrôle X1");
		return;
	} else if (!edit_cpy1.value.match(/^-?\d+$/) &&
		   !edit_cpy1.value.match(/^-?\d+\.\d+$/)) {
		alert("Mauvaise valeur de point de contrôle Y1");
		return;
	} else if (!edit_cpx2.value.match(/^-?\d+$/) &&
		   !edit_cpx2.value.match(/^-?\d+\.\d+$/)) {
		alert("Mauvaise valeur de point de contrôle X2");
		return;
	} else if (!edit_cpy2.value.match(/^-?\d+$/) &&
		   !edit_cpy2.value.match(/^-?\d+\.\d+$/)) {
		alert("Mauvaise valeur de point de contrôle Y2");
		return;
	...
	var str = "<bezier x1=\"" + xmlEncode(edit_x1.value) + unit_x1 + "\" ";
	str += "y1=\"" + xmlEncode(edit_y1.value) + unit_y1 + "\" ";
	str += "x2=\"" + xmlEncode(edit_x2.value) + unit_x2 + "\" ";
	str += "y2=\"" + xmlEncode(edit_y2.value) + unit_y2 + "\" ";
	str += "cp-x1=\"" + xmlEncode(edit_cpx1.value) + unit_cpx1 + "\" ";
	str += "cp-y1=\"" + xmlEncode(edit_cpy1.value) + unit_cpy1 + "\" ";
	str += "cp-x2=\"" + xmlEncode(edit_cpx2.value) + unit_cpx2 + "\" ";
	str += "cp-y2=\"" + xmlEncode(edit_cpy2.value) + unit_cpy2 + "\" ";
