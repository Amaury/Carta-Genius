...
/*
** ys_popword()
** Extract the first word of a ystring.
*/
ystr_t ys_popword(ystr_t s)
{
  ystr_head_t *y;
  char *pt;
  ystr_t ys;
  ...
  if (!(ys = ys_new("")))
    return (NULL);
  for (pt = s; !IS_SPACE(*pt) && *pt && (pt - s) < y->used; ++pt)
    {
      ys_addc(&ys, *pt);
      *pt = ' ';
    }
...
/*
** ys_getword()
** Extract the last word of a ystring.
*/
ystr_t ys_getword(ystr_t s)
{
  ystr_head_t *y;
  char *pt;
  ystr_t ys;
  ...
  if (!(ys = ys_new("")))
    return (NULL);
  for (pt = s + y->used - 1; !IS_SPACE(*pt) && pt >= s; --pt)
    {
      ys_putc(&ys, *pt);
      y->used--;
    }
